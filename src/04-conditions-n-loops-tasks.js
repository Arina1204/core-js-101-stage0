...
function isCreditCardNumber(ccn) {
  const digit = ccn.toString().split('').map(Number);
  const withLast = digit.slice(0, -1);
  const reversArr = withLast.reverse();
  let sum = 0;
  for (let i = 0; i < reversArr.length; i += 1) {
    if (i % 2 === 0) {
      reversArr[i] *= 2;
      if (reversArr[i] > 9) reversArr[i] -= 9;
    }
    sum += reversArr[i];
  }
  return digit.pop() === (10 - (sum % 10)) % 10;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === '(' || str[i] === '{' || str[i] === '[' || str[i] === '<') {
      stack.push(str[i]);
    } else if (stack.length === 0 || pairs[stack.pop()] !== str[i]) {
      return false;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < first.length; i += 1) {
    const part = first[i];
    const allMatch = parts.every((p) => p[i] === part);
    if (allMatch) {
      common.push(part);
    } else {
      break;
